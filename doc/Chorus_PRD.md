...
**输出：`reality_discrepancies`**

从叙述中提取值得搜索的对象，通过 Tavily 联网搜索获取客观信息，整合为结构化发现列表传给所有 Agent。每条发现含：涉及实体、用户原话（未提及则标注"未提及"）、核实事实、严重程度（low / medium / high）。

两类情况均会标记：用户陈述与现实存在差异，以及用户未提及但对决策有参考价值的重要背景信息。

**双重性质：** 认知落差同时是事实信号（更新 Agent 的信息基础）和心理信号（用户为何如此感知本身有意义）。每个 Agent 基于各自心理学维度独立解读，Chorus 不用外部事实覆盖内在感受。

**触发与降级：** 叙述中存在可搜索对象时触发。无 `TAVILY_API_KEY`、无可提取实体或搜索异常时，`reality_discrepancies` 为空，图继续执行，不阻塞后续节点。
